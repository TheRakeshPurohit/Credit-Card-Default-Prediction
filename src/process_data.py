...
class DataPrepare:
    ...
    def process_data(self):
        ...
        train_data = data_process.encode_categorical_data(
            train_data, ["customer_id", "name"]
        )

        data_test_process = DataProcessingTest(test_data)
        null_present, cols_with_missing_values = data_test_process.is_null_present()
        test_data = data_test_process.impute_missing_values(
            test_data, cols_with_missing_values
        )
        test_data = data_test_process.encode_categorical_data(
            test_data, ["customer_id", "name"]
        )
        return train_data, test_data

    def process_scaled_data(self, train_data, test_data):
        data_process = DataProcessingTest(train_data)
        null_present, cols_with_missing_values = data_process.is_null_present()
        train_data = data_process.impute_missing_values(
            train_data, cols_with_missing_values
        ...
        train_scaled_data = data_process.prepare_df_for_new_scaled_features(train_data)
        ...
        test_scaled_data = data_test_process.prepare_df_for_new_scaled_features(
            test_data
        )
